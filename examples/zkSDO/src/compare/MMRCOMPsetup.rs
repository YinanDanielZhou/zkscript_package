mod mmr_circuit_comp;
use mmr_circuit_comp::MmrCompareCircuit;
use mmr_circuit_comp::MAX_MERGE_COUNT;
...
fn proof_generate_and_verify(
    ...
    let proof_parameters= read_parameters("src/compare/proof_parameters.json")?;
    let t = s.elapsed();
    ...
    let operator = ScalarFieldBls::from(proof_parameters.10);

    let circuit = MmrCompareCircuit {
        mmr_root: root,
        mmr_root_2: root_2,
    ...
    let public_inputs: Vec<ScalarFieldBls> = vec![circuit.mmr_root, circuit.path_config_field, circuit.mmr_root_2, circuit.path_config_field_2, circuit.operator];
    ...
    save_to_file(&proof,"proof/MMRCOMP/proof.json","proof")?;
    save_to_file(&public_inputs, "proof/MMRCOMP/public_inputs.json","public_inputs")?;
...
fn main() -> Result<(), Box<dyn std::error::Error>>{
    ...
    let operator = ScalarFieldBls::from(0u32); // 0 : <, 1 : <=, 2 : >, 3 : >=, 4 : ==, 5 : !=
    ...
    save_to_file(&vk, "setup_results/MMRCOMP/verifying_key.json", "verifying_key")?;
    let do_proving: bool = true;
    if !do_proving {
        let s1 = std::time::Instant::now();
        save_to_file_binary(&pk, "setup_results/MMRCOMP/proving_key.bin")?;
        let t1 = s1.elapsed();
...
fn read_parameters(path: &str) -> Result<([u8; 32], [u8; 32], i64, Vec<[u8; 32]>, Vec<bool>, Vec<bool>, i64, Vec<[u8; 32]>, Vec<bool>, Vec<bool>, u8), Box<dyn std::error::Error>>{
    ...
    let operator: u8 = json_data["operator"].as_u64().ok_or("operator must be u8")? as u8;

    Ok((root, root_2, leaf_preimage, path_hashes, path_dirs, skip_hashing, leaf_preimage_2, path_hashes_2, path_dirs_2, skip_hashing_2, operator))
}

...
fn main() -> Result<(), Box<dyn std::error::Error>>{
    ...
    let preimage_hello = read_parameter("parameter_hello.json");
    let preimage_world = read_parameter("parameter_world.json");

    // Build the circuit
    let circuit_hello = Sha256Preimage::<Fr> {
        preimage: preimage_hello.as_bytes().to_vec(),
        hash: input_to_vec_hash(&preimage_hello),
    };

    let circuit_world = Sha256Preimage::<Fr> {
        preimage: preimage_world.as_bytes().to_vec(),
        hash: input_to_vec_hash(&preimage_world),
    };

    // Setup
    let (pk, vk) = Groth16::<Bls12_381>::circuit_specific_setup(circuit_hello.clone(), &mut rng)
        .map_err(|e| format!("Setup failed: {}",e))?;

    // Proving
    let proof_hello = Groth16::<Bls12_381>::prove(&pk, circuit_hello.clone(), &mut rng)
        .map_err(|e| format!("Proof generation failed: {}", e))?;

    let proof_world = Groth16::<Bls12_381>::prove(&pk, circuit_world.clone(), &mut rng)
        .map_err(|e| format!("Proof generation failed: {}", e))?;

    // Verifying
    let is_hello_valid = Groth16::<Bls12_381>::verify(&vk, &input_to_vec_hash(&preimage_hello), &proof_hello)
        .map_err(|e| format!("Verification failed: {}", e))?;
    assert!(is_hello_valid,"Proof for Hello is invalid");

    let is_world_valid = Groth16::<Bls12_381>::verify(&vk, &input_to_vec_hash(&preimage_world), &proof_world)
        .map_err(|e| format!("Verification failed: {}", e))?;
    assert!(is_world_valid,"Proof for World is invalid");

    // Save proof, verification key, and public input to files
    save_to_file(&proof_hello,"proof/proof_hello.json","proof")?;
    save_to_file(&proof_world,"proof/proof_world.json","proof")?;
    save_to_file(&vk, "proof/verifying_key.json","verifying_key")?;
    save_to_file(&input_to_vec_hash(&preimage_hello), "proof/public_inputs_hello.json","public_inputs")?;
    save_to_file(&input_to_vec_hash(&preimage_world), "proof/public_inputs_world.json","public_inputs")?;
